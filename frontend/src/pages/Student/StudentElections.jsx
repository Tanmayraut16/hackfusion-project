...
const StudentElections = () => {
  ...
  useEffect(() => {
    const fetchElections = async () => {
      setLoading(true);
      try {
        const response = await fetch("http://localhost:3000/api/election/elections");
        ...
        setElections({
          ongoing: data.filter(e => new Date(e.startDate) <= now && new Date(e.endDate) >= now),
  ...
  const fetchElectionById = async (id) => {
    ...
    setError("");
    try {
      const response = await fetch(`http://localhost:3000/api/election/elections/${id}`);
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {selectedElection.positions.map((position) => (
                <div key={position._id} className="bg-white rounded-lg shadow-md p-6">
